...
async function createSingleUser(
  ...
  const roleChoice = await question("ロール番号を入力してください (1-3): ");
...
async function createOrganizationUsers() {
  ...
  try {
    ...
    if (createOrg.toLowerCase() === 'y' || createOrg.toLowerCase() === 'yes') {
      ...
      if (!orgName.trim()) {
        console.log("❌ 組織名が入力されていません。処理を終了します。");
        rl.close();
        process.exit(1);
      }
      ...
      if (existingOrgs.length > 0) {
        ...
        if (orgChoice.trim()) {
          ...
          if (isNaN(organizationId)) {
            console.log("❌ 無効な組織IDです。処理を終了します。");
            rl.close();
            process.exit(1);
          } else {
            ...
            if (selectedOrg) {
              ...
              console.log("❌ 指定された組織IDが見つかりません。処理を終了します。");
              rl.close();
              process.exit(1);
            }
          }
        } else {
          console.log("❌ 組織IDが必要です。処理を終了します。");
          rl.close();
          process.exit(1);
        }
      } else {
        console.log("❌ 既存の組織がありません。まず組織を作成してください。");
        rl.close();
        process.exit(1);
      }
    }
    ...
    if (organizationId === null) {
      console.log("❌ 組織IDが設定されていません。処理を終了します。");
      rl.close();
      process.exit(1);
    }
